...
class rgb:
    ...
    def __init__(self, r, g, b):
        self.r, self.g, self.b = self.colormodel((r, g, b), returnable=True)
    ...
    def channel(cls, channel: int, returnable=False) -> bool | int:
        """ Validate a RGB channel

        Arg:

            channel(int): The value of a RGB channel
        ...
        if not isinstance(channel, int) or not (0 <= channel <= 255): 
            ...
            elif returnable: raise Exception(f"{channel} is NOT a valid RGB channel.")

        else: return channel if returnable else True

    @classmethod
    def colormodel(cls, colormodel: tuple, returnable=False) -> None | tuple:
        """ Validate a RGB color model

        Args:

            channels(tuple[int, int, int]): 
                A tuple of channels, each representing a channel in a RGB color model.

            returnable(bool, optional): 
                Determine the return type. If True, return the received RGB color model as a tuple if valid, otherwise 
        ...
        if not isinstance(colormodel, tuple):
            ...
            if not returnable: return False
                ...
                f"Received color model: {colormodel} is not in valid RGB color model format: (r, g, b)."
            )

        try:
            for channel in colormodel:rgb.channel(channel, returnable=True)
